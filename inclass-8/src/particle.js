...
const update = ({acceleration, velocity, position, mass}, delta, canvas) => {
	// IMPLEMENT ME
	position[0] = position[0] + delta * velocity[0] + 1/2 * acceleration[0] * delta * delta
	position[1] = position[1] + delta * velocity[1] + 1/2 * acceleration[1] * delta * delta
	if (position[0] > 800 + mass || position[0] < 0 - mass) {
		while (position[0] < 0) {
			position[0] += 800
		}
		position[0] = position[0] % 800 
	}
	if (position[1] > 800 + mass || position[1] < 0 - mass) {
		while (position[1] < 0) {
			position[1] += 800
		}
		position[1] = position[1] % 800
	}
